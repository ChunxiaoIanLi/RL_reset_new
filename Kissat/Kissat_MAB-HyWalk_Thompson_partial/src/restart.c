...
kissat_restart (kissat * solver)
{
  ...
  if(level == 0){
    ...
    if (solver->resetPrevLever == 0){
      ...
      // Remember the order of the k literals with the top activities
      static const unsigned max_literals_to_preserve = 5;
      unsigned literals_to_preserve[max_literals_to_preserve]; // Note: this memory is always written before it is read
      unsigned num_literals_to_preserve = 0;

      for (unsigned i = 0; i < max_literals_to_preserve && !kissat_empty_heap (&solver->scores); i++, num_literals_to_preserve++) {
        // Get the next unassigned variable with the largest activity
        unsigned idx;
        const value *values = solver->values;
        do {
          idx = kissat_max_heap (&solver->scores);
          kissat_pop_heap (solver, &solver->scores, idx);
        } while (values[LIT (idx)]);
        literals_to_preserve[i] = idx;
      }

      // Push the k variables back onto the heap
      for (unsigned i = 0; i < num_literals_to_preserve; i++) {
        const unsigned idx = literals_to_preserve[i];
        kissat_push_heap (solver, &solver->scores, idx);
      }

      // Reset activities
      const double max_activity = 1.0;

      for (all_variables(idx)){
        kissat_update_heap(solver, &solver->scores, idx, (double) rand() / RAND_MAX * max_activity);
      }

      // Ensure the original top k variables still have the top k activities
      const double epsilon = 0.001;
      for (unsigned i = 0; i < num_literals_to_preserve; i++) {
        const unsigned idx = literals_to_preserve[i];
        const double new_activity = max_activity + epsilon * (num_literals_to_preserve - i);
        kissat_update_heap(solver, &solver->scores, idx, new_activity);
      }

      kissat_update_scores(solver);
      // for (all_variables(idx)){
      //   const double score = kissat_get_heap_score (&solver->scores, idx);
      //   printf("%.2g ", score);
      // }
      // printf("\n");
      solver->nof_resets++;
